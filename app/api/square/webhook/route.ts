...
async function grantRandomizerOrder(squareOrderId: string, squarePaymentId: string) {
  ...
  const { error: accountError } = await supabase.from("randomizer_accounts").upsert({
    ...
    lifetime_access:
      Boolean(account?.lifetime_access) || Boolean(order.lifetime_access),
    access_expires_at: order.lifetime_access
      ? account?.access_expires_at || null
      : nextExpiry?.toISOString() || null,
